...
void timer0_OC0_init(){ //in the main we should init this first before the timer
    setPIN_dir(B,OC0, OUT); //make sure we set it to out put first;
}
...
void timer2_OC2_init(){ //in the main we should init this first before the timer
    setPIN_dir(D,OC2, OUT); //make sure we set it to output first;
}
...
void timer1_OC1A_init(){
    setPIN_dir(D,OC1A, OUT); //make sure we set it to output first;
}
/**
 * 
...
void timer1_OC1B_init(){
    setPIN_dir(D,OC1B, OUT); //make sure we set it to output first;
}
/**
 * 
...
void timer1_ICP_init(){
    setPIN_dir(D,ICP1, IN); //make sure we set it to input first;
    ACSR &= ~(1<<ACIC);
}
...
void timer1_ACO_init(){
    setPIN_dir(B,AIN0, IN);
    setPIN_dir(B,AIN1, IN);
    ACSR |= (1<<ACIC);
}
